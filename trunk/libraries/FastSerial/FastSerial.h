...
///
class FastSerial: public BufferedStream {
public:
	...
	//virtual size_t writeBlock(uint8_t *buf, uint16_t length);
#else
	virtual void write(uint8_t c);
	//virtual void writeBlock(uint8_t *buf, uint16_t length);
#endif
...
{                                                                       \
        ...
        i = (__FastSerial__rxBuffer[_PORT]._head + 1) & __FastSerial__rxBuffer[_PORT]._mask; \
        /* decide whether we have space for another byte */             \
        if (i != __FastSerial__rxBuffer[_PORT]._tail) {                 \
                /* we do, move the head */                              \
                __FastSerial__rxBuffer[_PORT]._bytes[__FastSerial__rxBuffer[_PORT]._head] = c; \
                __FastSerial__rxBuffer[_PORT]._head = i;                \
        }                                                               \
}                                                                       \
ISR(_TXVECTOR, ISR_BLOCK)                                               \
{                                                                       \
        /* if there is another character to send */                     \
        if (__FastSerial__txBuffer[_PORT]._tail != __FastSerial__txBuffer[_PORT]._head) { \
                _UDR = __FastSerial__txBuffer[_PORT]._bytes[__FastSerial__txBuffer[_PORT]._tail]; \
                /* increment the tail */                                \
                __FastSerial__txBuffer[_PORT]._tail =                    \
                        (__FastSerial__txBuffer[_PORT]._tail + 1) & __FastSerial__txBuffer[_PORT]._mask; \
        } else {                                                        \
                /* there are no more bytes to send, disable the interrupt */ \
                if (__FastSerial__txBuffer[_PORT]._head == __FastSerial__txBuffer[_PORT]._tail) \
                        _UCSRB &= ~_TXBITS;                             \
        }                                                               \
}                                                                       \
struct hack
